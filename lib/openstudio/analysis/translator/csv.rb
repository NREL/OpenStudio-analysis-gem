    ...
    module Translator
      class CSV
        ...
        def process
          ...
          @outputs = json.dumps(@outputs) #call json here
        ...
        def parse_csv
          # Check that the configuration row is convertable to a hash
          config_row = @csv[0]
          ...
          fail "Require setting not found: measure_path" unless config_hash[:measure_path]
        ...
        def parse_rows
          ...
          measures.each_with_index do |measure, measure_index|
            data[measure_index] = {}
            measure_json = ''
            while i in 0..(length(@measure_paths))
              if File.exists? File.join(@measure_paths[i],measure.to_s,"measure.json")
                measure_json = MultiJson.load(File.join(@measure_paths[i],measure.to_s,"measure.json"))
                break
              end
            end
            ...
            vars.each do |var|
              var_hash = {}
              var_json = measure_json['arguments'].select{|hash| hash['local_variable'] == var.to_s}
              ...
              case var_hash[:type]
                when 'bool', 'boolean' # is 'boolean' necesary? it's not in the enum catch
                  var_hash[:distribution][:values] = (4..(length(csv)-1)).map{|value| value.to_s == 'true'}
                ...
                when 'choice'
                  var_hash[:distribution][:values] = (4..(length(csv)-1)).map{|value| value.to_s}
                  ...
                  var_hash[:distribution][:values] = (4..(length(csv)-1)).map{|value| eval(value)}
              ...
              var_hash[:distribution][:weights] = eval("[" + "#{1.0/(length(csv)-4)}" * (length(csv)-4) + "]")
